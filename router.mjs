...
router.get("/callback", async function handleSingpassCallback(ctx) {
  try {
    ...
    ctx.redirect(
      process.env.FRONTEND_URL || "https://kits-web-app.netlify.app/"
    );
  } catch (err) {
...
router.get("/user", function getUser(ctx) {git remote add origin https://github.com/gittyupkit/test-app-backend.git
  if (ctx.session.user) {
...
router.get("/logout", function handleLogout(ctx) {
  ctx.session = null;
  ctx.redirect(process.env.FRONTEND_URL || "https://YOUR-FRONTEND-URL/");
});
